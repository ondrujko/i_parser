...
class Scrap:
    ...
    def scrap(self, usernames, count, kill_edge=1000):
        ...
        # subprocess.Popen('~/PycharmProjects/python/killall firefox.sh')
        ...
        def num_of_posts():
            try:  # How many posts in this account
                self.number_of_posts = self.driver.find_element_by_class_name('g47SY').text.replace(',', '').replace('.','').replace(' ', '')
                print(self.number_of_posts)
            except:
                try:
                    self.number_of_posts = self.driver.find_element_by_class_name('g47SY lOXF2').text.replace(' ', '').replace(',','').replace('.','')
                    print(self.number_of_posts)
                except:
                    pass
