...
def top(message):
    subprocess.run('top')
    bot.send_message(message.chat.id, 'Топ выведена')
...
def restart(message):
    ...
    if sc.saved(sc.marks):
        ...
        bot.send_message(message.chat.id, "Не удалось сохранить данные.")
    subprocess.run(['killall', 'firefox-bin'])
    bot.stop_polling()
    bot.stop_polling()
...
def msg(message):
    ...
    if message.text == '🔎Инстаграм парсинг':
        if sc.process_busy is False:
            ...
            bot.send_message(message.chat.id,
                             'Занято незаверешенным процессом!\nДождитесь завершения или перезапустите бота командой '
                             '/restart')
    ...
    if message.text == '📥Запросить уже найденные аккаунты':
        try:
            ...
            print(e)
            bot.send_message(message.chat.id, 'Сейчас нет активных процессов')
        ...
        try:
            if sc.saved(sc.marks):
                try:
                    file = open('result_parser.txt', 'rb')
                    try:
                        bot.send_message(message.chat.id, f'Готово!\n<b>Отметок было найдено:</b> {len(sc.marks)}',
                                         parse_mode='html')
                    except Exception as e:
                        print(e)
